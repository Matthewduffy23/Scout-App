...
export const POSITION_LABELS = {
  ...
  'DMF':'Defensive Midfielder (DM)', 'LDMF':'Defensive Midfielder (DM)', 'RDMF':'Defensive Midfielder (DM)',
  'LCMF':'Central Midfielder (CM)', 'RCMF':'Central Midfielder (CM)',
  'AMF':'Attacking Midfielder (AM)',
  'RAMF':'Right Winger (RW)', 'RWF':'Right Winger (RW)', 'RW':'Right Winger (RW)',
  'LAMF':'Left Winger (LW)', 'LWF':'Left Winger (LW)', 'LW':'Left Winger (LW)',
  'CF':'Striker (CF)',
};
const LEAGUE_DISPLAY_NAMES = {
  'England 1.':'Premier League','England 2.':'Championship','England 3.':'League One',
  'England 4.':'League Two','England 5.':'National League',
  'Spain 1.':'La Liga','Spain 2.':'Segunda División',
  'Germany 1.':'Bundesliga','Germany 2.':'2. Bundesliga','Germany 3.':'3. Liga',
  'Italy 1.':'Serie A','Italy 2.':'Serie B',
  'France 1.':'Ligue 1','France 2.':'Ligue 2',
  'Portugal 1.':'Primeira Liga','Portugal 2.':'Segunda Liga',
  'Netherlands 1.':'Eredivisie','Belgium 1.':'Pro League',
  'Scotland 1.':'Scottish Premiership','Scotland 2.':'Championship',
  'Turkey 1.':'Süper Lig','Greece 1.':'Super League Greece',
  'Brazil 1.':'Brasileirao Serie A','Argentina 1.':'Liga Profesional',
  'USA 1.':'MLS','Mexico 1.':'Liga MX',
  'Japan 1.':'J1 League','Korea 1.':'K League 1',
  'Saudi 1.':'Saudi Pro League','Russia 1.':'Premier League',
  'Ukraine 1.':'Premier League','Poland 1.':'Ekstraklasa',
  'Austria 1.':'Austrian Bundesliga','Switzerland 1.':'Super League',
  'Denmark 1.':'Superliga','Sweden 1.':'Allsvenskan',
  'Norway 1.':'Eliteserien','Croatia 1.':'HNL',
  'Czech 1.':'Chance Liga','Serbia 1.':'SuperLiga',
  'Romania 1.':'SuperLiga','Egypt 1.':'Egyptian Premier League',
  'Morocco 1.':'Botola Pro','Australia 1.':'A-League Men',
};
const TEAM_PLACEHOLDER = {
  possession: 54, verticality: 47, pressing: 61, attack: 58, passing: 66,
};
...
export function rolesDotsSvg(sortedRoles, w = 880, rowH = 56) {
  ...
  const rows = sortedRoles.map(([role, score], i) => {
...
// ─── Pentagon radar for team identity (placeholder data) ────────────────────
function teamRadarSvg(values, size = 290) {
  const labels = ['Possession', 'Verticality', 'Pressing', 'Attack', 'Passing'];
  const keys = ['possession', 'verticality', 'pressing', 'attack', 'passing'];
  const cx = size / 2, cy = size / 2 - 6;
  const R = size * 0.34;
  const N = 5;
  const angle = (i) => -Math.PI / 2 + (i * 2 * Math.PI) / N;

  const ringPts = (r) => keys.map((_, i) => {
    const a = angle(i);
    return `${cx + r * Math.cos(a)},${cy + r * Math.sin(a)}`;
  }).join(' ');

  const rings = [0.25, 0.5, 0.75, 1].map(f =>
    `<polygon points="${ringPts(R * f)}" fill="none" stroke="#1e2a3e" stroke-width="1"/>`
  ).join('');

  const spokes = keys.map((_, i) => {
    const a = angle(i);
    return `<line x1="${cx}" y1="${cy}" x2="${cx + R * Math.cos(a)}" y2="${cy + R * Math.sin(a)}" stroke="#1e2a3e" stroke-width="1"/>`;
  }).join('');

  const dataPts = keys.map((k, i) => {
    const a = angle(i);
    const v = Math.max(0, Math.min(100, values[k] || 50)) / 100;
  ...
  const dots = keys.map((k, i) => {
    ...
    return `<circle cx="${x}" cy="${y}" r="4.5" fill="#a78bfa" stroke="#07090f" stroke-width="1.5"/>`;
  }).join('');
  ...
  const labelEls = keys.map((k, i) => {
    ...
    const v = values[k] || 50;
    return `<text x="${x}" y="${y - 6}" font-family="Montserrat,sans-serif" font-size="13" font-weight="700" fill="#c8d2e0" text-anchor="middle">${labels[i]}</text>
            <text x="${x}" y="${y + 10}" font-family="Montserrat,sans-serif" font-size="14" font-weight="900" fill="#a78bfa" text-anchor="middle">${v}</text>`;
  }).join('');

  return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
    ${rings}
    ${spokes}
    <polygon points="${dataPts}" fill="rgba(167,139,250,0.22)" stroke="#a78bfa" stroke-width="2"/>
    ${dots}
    ${labelEls}
  </svg>`;
}
...
function ensureMontserratEmbedded() {
  ...
  _montserratEmbedPromise = (async () => {
    if (!document.getElementById('qc-montserrat-embed')) {
      const style = document.createElement('style');
      style.id = 'qc-montserrat-embed';
    ...
    return `
      <div style="margin-bottom:11px;">
        ...
        <div style="position:relative;height:10px;background:#1b2636;border-radius:5px;margin-bottom:4px;">
          ...
          <div style="position:absolute;top:-3px;left:${pAvg}%;width:2px;height:16px;background:#5e6678;transform:translateX(-50%);"></div>
...
function buildQuickCardElement(player, players, manual = {}) {
  ...
  // crest silently shows whichever club player.teamFotmobId points to, regardless
  // of which club's stats/name are actually displayed. Same fix as PlayerScoutingCard.js.
  const _normTeam = (s) => s ? s.normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/\s+/g,' ').trim() : '';
...
export default function QuickCardModal({ player, players, onClose }) {
  ...
  const handlePhotoUpload = (e) => {
    ...
    reader.onload = (ev) => setUploadedPhotoDataUrl(ev.target.result);
